...
// Layout for the data pointed to:
// union {
//     struct has_value {
//         u64 {
//             SET_TO_ZERO : u1
...
impl GCRational {
    ...
    fn numerator_and_denominator_from_metadata(
        ...
        let denominator_ptr = numerator_ptr
            + (metadata.numerator_len as usize * std::mem::size_of::<limb_t>())
                .div_ceil(GarbageCollector::BLOCK_SIZE);
...
impl GCDebug for GCRational {
    unsafe fn gc_debug(
        ...
        unsafe {
            write!(f, "{}", numerator.to_naive_string(gc))?;

            write!(f, "/")?;

            write!(f, "{}", denominator.to_naive_string(gc))
        }
    }
}
